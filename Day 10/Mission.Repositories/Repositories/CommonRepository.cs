...
namespace Mission.Repositories.Repositories
{
    public class CommonRepository(MissionDbContext cIDbContext) : ICommonRepository
    {
        ...
        public List<DropDownResponseModel> MissionTypeList()
        {
            var missionTypes = _cIDbContext.Missions
                .Where(m => !m.IsDeleted)
                .Select(m => m.MissionType)
                .Distinct()
                .OrderBy(t => t)
                .Select((t, index) => new DropDownResponseModel(index + 1, t))
                .ToList();

            return missionTypes;
        }

        public Dictionary<string, int> GetActiveMissionCountByTheme()
        {
            var missionCounts = _cIDbContext.Missions
                .Where(m => !m.IsDeleted)
                .GroupBy(m => m.MissionTheme.ThemeName)
                .Select(g => new
                {
                    ThemeName = g.Key,
                    Count = g.Count()
                })
                .ToDictionary(x => x.ThemeName, x => x.Count);
