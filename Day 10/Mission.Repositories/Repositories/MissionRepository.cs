...
namespace Mission.Repositories.Repositories
{
    public class MissionRepository(MissionDbContext dbContext) : IMissionRepository
    {
        ...
        public async Task<List<Missions>> SearchMissionsByKeyword(string keyword)
        {
            if (string.IsNullOrWhiteSpace(keyword))
                return await dbContext.Missions.Where(m => !m.IsDeleted).ToListAsync();
            
            return await dbContext.Missions
                .Where(m => !m.IsDeleted && 
                       (m.MissionTitle.Contains(keyword) || 
                        m.MissionDescription.Contains(keyword) ||
                        m.MissionOrganisationName.Contains(keyword)))
                .ToListAsync();
        }

        public async Task<bool> IsMissionTitleExist(string title)
        {
            return await dbContext.Missions
                .AnyAsync(m => m.MissionTitle.ToLower() == title.ToLower() && !m.IsDeleted);
        }
    }
}
