...
namespace Mission.Repositories.Repositories
{
    public class MissionSkillRepository(MissionDbContext cIDbContext) : IMissionSkillRepository
    {
        ...
        public List<MissionSkillResponseModel> GetMissionSkillsByStatus(string status)
        {
            return _cIDbContext.MissionSkills
                .Where(ms => !ms.IsDeleted && ms.Status.ToLower() == status.ToLower())
                .Select(ms => new MissionSkillResponseModel()
                {
                    Id = ms.Id,
                    SkillName = ms.SkillName,
